            ...
            <div className=" bg-gray-100 dark:bg-gray-900 flex flex-col md:flex-row-reverse space-y-4 md:space-y-0 md:space-x-4">
                ...
                {isModalOpen && (
                    <div className="fixed inset-0 z-9999 flex justify-center items-center bg-gray-900 bg-opacity-50">
                        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark w-1/3 p-6 max-h-[90vh] overflow-y-auto">
                            <h4 className="text-xl font-semibold text-black dark:text-white">Input Data Pelanggan</h4>

                            {/* Form Input */}
                            <div className="space-y-4 mt-6">
                                {[
                                    { label: 'Nama Pelanggan', type: 'text', placeholder: 'Enter Nama Pelanggan' },
                                    { label: 'Alamat', type: 'text', placeholder: 'Enter Alamat Pelanggan' },
                                    { label: 'Kontak', type: 'text', placeholder: 'Enter Kontak Pelanggan' },
                                    { label: 'Email', type: 'email', placeholder: 'Enter Email Pelanggan' },
                                ].map((field, index) => (
                                    <div key={index}>
                ...
                <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark w-full">
                    <div className="px-4 py-6 md:px-6 xl:px-7.5">
                        <h4 className="text-xl font-semibold text-black dark:text-white">Data Pelanggan</h4>
                    </div>
                    ...
                    <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 border-separate border-spacing-0 border border-gray-300 dark:border-gray-700">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    {['No', 'Nama Pelanggan', 'Alamat', 'Kontak', 'Email', 'Action'].map((header, index) => (
                            ...
                            <tbody>
                                {customerData.map((customer, index) => (
                                    <tr key={index} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                        ...
                                        <td className="px-6 py-4 text-sm text-black dark:text-white">{customer.namaPelanggan}</td>
                                        <td className="px-6 py-4 text-sm text-black dark:text-white">{customer.alamat}</td>
                                        <td className="px-6 py-4 text-sm text-black dark:text-white">{customer.kontak}</td>
                                        <td className="px-6 py-4 text-sm text-black dark:text-white">{customer.email}</td>
